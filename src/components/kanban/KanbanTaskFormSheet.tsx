        ...
        >
          ...
          <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-5 py-4">
            <div className="space-y-5">
              ...
              {/* Status */}
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-(--duocal-text)">Status</label>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          ativo ? '' : 'bg-(--duocal-surface-soft)',
                        )}
                        style={ativo ? {
                          backgroundColor: cfg.bg,
                          color: cfg.cor,
                          boxShadow: `0 0 0 2px ${cfg.cor}`,
              ...
              {/* Prioridade */}
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-(--duocal-text)">Prioridade</label>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          ativo ? '' : 'bg-(--duocal-surface-soft) text-(--duocal-muted)',
                        )}
              ...
              <div className="space-y-1.5">
                ...
                <button
                  ...
                  className="flex h-12 w-full cursor-pointer items-center justify-between rounded-[18px] border border-(--duocal-border) bg-(--duocal-surface-soft) px-4 text-sm text-(--duocal-text) transition hover:border-(--duocal-primary)"
                >
                  <span className={cn(!form.dtPrazo && 'text-(--duocal-muted)')}>
                    {formatDateDisplay(form.dtPrazo)}
                  </span>
                  <Calendar className="size-[18px] shrink-0 text-(--duocal-muted)" />
                </button>
                {form.dtPrazo && (
                  <button
                    type="button"
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {categorias.map((c) => (
                      <button
                        ...
                        className={cn(
                          'rounded-2xl px-3.5 py-2 text-xs font-semibold transition',
                          form.categoriaId === c.id ? '' : 'bg-(--duocal-surface-soft)',
                        )}
                        style={
                          form.categoriaId === c.id
                            ? { backgroundColor: c.cd_cor + '22', color: c.cd_cor, boxShadow: `0 0 0 2px ${c.cd_cor}` }
                            : { color: 'var(--duocal-muted)' }
                        }
                      >
                        {c.nm_categoria}
                    ...
                    <div className="rounded-2xl border border-red-200 bg-red-50 p-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 rounded-xl bg-(--duocal-surface-soft) py-2 text-xs font-semibold text-(--duocal-muted)"
                        >
                          Cancelar
                        </button>
                        <button
